...
オーディオ、RGB アンダーグロー、バックライト、キーの動作に問題がある場合は、NVRAM (永続的な設定のストレージ)をリセットすることができます。NVRAM を強制的にリセットするには、[`EEP_RST` キーコード](ja/quantum_keycodes.md)あるいは[ブートマジック](ja/feature_bootmagic.md)機能を使います。それらのいずれも選択肢にない場合は、カスタムマクロを使って行うことができます。

NVRAM を消去するには、関数またはマクロから `nvconfig_init()` を実行し、ほとんどの設定をデフォルトにリセットします。
