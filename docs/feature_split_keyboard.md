...
This method sets the keyboard's handedness by setting a flag in the persistent storage (`NVRAM`).  This is checked when the controller first starts up, and determines what half the keyboard is, and how to orient the keyboard layout. 
...
Next, you will have to flash the NVRAM files once for the correct hand to the controller on each halve. You can do this manually with the following bootloader targets while flashing the firmware:
...
?> ARM controllers using `dfu-util` will require an NVRAM reset after setting handedness. This can be done using the `EEP_RST` keycode or [Bootmagic Lite](feature_bootmagic.md). Controllers using emulated NVRAM will always require handedness parameter when flashing the firmware.

?> [QMK Toolbox]() can also be used to flash NVRAM handedness files. Place the controller in bootloader mode and select menu option Tools -> NVRAM -> Set Left/Right Hand

This setting is not changed when re-initializing the NVRAM using the `EEP_RST` key, or using the `nvconfig_init()` function.  However, if you reset the NVRAM outside of the firmware's built in options (such as flashing a file that overwrites the `NVRAM`, like how the [QMK Toolbox]()'s "Reset EEPROM" button works), you'll need to re-flash the controller with the `NVRAM` files. 

You can find the `NVRAM` files in the QMK firmware repo, [here](https://github.com/qmk/qmk_firmware/tree/master/quantum/split_common).
