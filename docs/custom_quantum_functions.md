...
void keyboard_post_init_user(void) {
  ...
  rgblight_mode_no_nvram(RGBLIGHT_MODE_BREATHING + 3); // sets mode to Fast breathing without saving
}
```
...
# Persistent Configuration (NVRAM)

This allows you to configure persistent settings for your keyboard.  These settings are stored in the NVRAM of your keyboard, and are retained even after power loss. The settings can be read with `nvconfig_read_kb` and `nvconfig_read_user`, and can be written to using `nvconfig_update_kb` and `nvconfig_update_user`. This is useful for features that you want to be able to toggle (like toggling rgb layer indication).  Additionally, you can use `nvconfig_init_kb` and `nvconfig_init_user` to set the default values for the NVRAM. 

The complicated part here, is that there are a bunch of ways that you can store and access data via NVRAM, and there is no "correct" way to do this.  However, you only have a DWORD (4 bytes) for each function.

Keep in mind that NVRAM has a limited number of writes. While this is very high, it's not the only thing writing to the NVRAM, and if you write too often, you can potentially drastically shorten the life of your board's persistent storage.
...
This sets up a 32 bit structure that we can store settings with in memory, and write to the NVRAM. Using this removes the need to define variables, since they're defined in this structure. Remember that `bool` (boolean) values use 1 bit, `uint8_t` uses 8 bits, `uint16_t` uses up 16 bits.  You can mix and match, but changing the order can cause issues, as it will change the values that are read and written. 

We're using `rgb_layer_change`, for the `layer_state_set_*` function, and use `keyboard_post_init_user` and `process_record_user` to configure everything. 

Now, using the `keyboard_post_init_user` code above, you want to add `nvconfig_read_user()` to it, to populate the structure you've just created. And you can then immediately use this structure to control functionality in your keymap.  And It should look like: 
```c
void keyboard_post_init_user(void) {
  // Call the keymap level matrix init.

  // Read the user config from NVRAM
...
```
The above function will use the NVRAM config immediately after reading it, to set the default layer's RGB color. The "raw" value of it is converted in a usable structure based on the "union" that you created above. 
  ...
  switch (keycode) {
    ...
    case RGB_LYR:  // This allows me to use underglow as layer indication, or as normal
        if (record->event.pressed) {
            ...
            nvconfig_update_user(user_config.raw); // Writes the new status to NVRAM
    ...
    case RGB_MODE_FORWARD ... RGB_MODE_GRADIENT: // For any of the RGB codes (see quantum_keycodes.h, L400 for reference)
        if (record->event.pressed) { //This disables layer indication, as it's assumed that if you're changing this ... you want that disabled
            if (user_config.rgb_layer_change) {        // only if this is enabled
                ...
                nvconfig_update_user(user_config.raw); // write the setings to NVRAM
            }
        }
        return true; break;
    default:
...
```
And lastly, you want to add the `nvconfig_init_user` function, so that when the NVRAM is reset, you can specify default values, and even custom actions. To force an NVRAM reset, use the `EEP_RST` keycode or [Bootmagic Lite](feature_bootmagic.md) functionallity. For example, if you want to set rgb layer indication by default, and save the default valued.

```c
void nvconfig_init_user(void) {  // NVRAM is getting reset!
  user_config.raw = 0;
  user_config.rgb_layer_change = true; // We want this enabled by default
  nvconfig_update_user(user_config.raw); // Write default value to NVRAM now

  // use the non noeeprom versions, to write these values to NVRAM too
...
* Keyboard/Revision: `void nvconfig_init_kb(void)`, `uint32_t nvconfig_read_kb(void)` and `void nvconfig_update_kb(uint32_t val)`
* Keymap: `void nvconfig_init_user(void)`, `uint32_t nvconfig_read_user(void)` and `void nvconfig_update_user(uint32_t val)`

The `val` is the value of the data that you want to write to NVRAM.  And the `nvconfig_read_*` function return a 32 bit (DWORD) value from the NVRAM. 
